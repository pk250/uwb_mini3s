...
//    dwt_readfromdevice(RX_FQUAL_ID, 0x0, 8, (uint8*)&diagnostics->stdNoise);
    ...
    diagnostics->firstPathAmp2 = dwt_read16bitoffsetreg(RX_FQUAL_ID, 2) ;
    diagnostics->firstPathAmp3 = dwt_read16bitoffsetreg(RX_FQUAL_ID, 4) ;
    diagnostics->maxGrowthCIR = dwt_read16bitoffsetreg(RX_FQUAL_ID, 6) ;
...
int _dwt_loaducodefromrom(void)
{
    ...
    Sleep(1); // Allow time for code to upload (should take up to 120 us)
		...
		dwt_writetodevice(PMSC_ID, PMSC_CTRL0_OFFSET, 2, wr_buf1);
